...
#            self.add_speaker(identifier, -32.5)
    ...
    def _get_matched_speaker(self):

        """ Gives the voice in the db which has least distance from the cluster
        wav as the best matched speaker """
        
        SpeakR = []
        ValUe = []
        for speaker in self.speakers:
            SpeakR.append(speaker)
            ValUe.append(abs(self.speakers[speaker]))

        print 'Speaker array',SpeakR
    ...
    def merge_waves(self):
        ...
        name = self.get_name()
        videocluster = os.path.join(dirname.split('.')[0], name)
        listwaves = os.listdir(videocluster)
        listwaves.sort()
        listw = [os.path.join(videocluster, fil) for fil in listwaves]
        self.wave = os.path.join(dirname.split('.')[0], name + ".wav")
    ...
    def print_segments(self):
        ...
        end_seg = []
        for seg in self._segments:
            start_seg.append(utils.humanize_time(float(seg.get_start()) / 100))
            end_seg.append(utils.humanize_time(float(seg.get_end()) / 100))
        return str(seg).split(' ')[-1].split(']')[0][1:-1], start_seg, end_seg
...
class Voiceid(object):
    ...
    def __init__(self, vdb, filename):
        """
        :type vdb: object
    ...
    def generate_seg_file(self, set_speakers=True):
        ...
        f_seg = open(self.get_file_basename() + '.ev_is.seg', 'w')
        f_seg.write(result)
        f_seg.close()
    ...
    def _extract_clusters(self):
        extract_clusters(self._basename + '.ev_is.seg', self._clusters)
...
#            self.automerge_clusters()
        ...
        if not interactive:
            if not quiet:
                ...
                for clu in self._clusters:
                    ...
                    print """Best matched speaker: %s"""%self[clu]._get_matched_speaker()
                    self._matched[clu] = self[clu]._get_matched_speaker().split('_')[-1]
    ...
    def update_db(self, t_num=4, automerge=False):
        ...
        def _build_model_wrapper(self, wave_b, cluster, wave_dir, new_speaker,
            ...
            try:
                utils.ensure_file_exists(wave_b + '.ev_is.seg')
            except IOError:
                self[cluster]._generate_a_seg_file(wave_b + '.ev_is.seg', wave_b)
            ...
            if old_speaker != new_speaker and old_speaker!='unknown':
                ...
                try:
                    utils.ensure_file_exists(old_b_file + '.ev_is.seg')
                except IOError:
                    clu._generate_a_seg_file(old_b_file + '.ev_is.seg', old_b_file)
...
#                    os.remove("%s.ident.seg" % wave_b )
                    ...
                    if os.path.exists(old_wav_name):
                        ...
                        os.remove(old_b_file + '.ev_is.seg')
                except OSError:
        ...
        thrds = {}
        if not os.path.exists(self.get_file_basename()+'.ev_is.seg'): self.generate_seg_file(set_speakers=False)
...
#                if not os.path.exists(wav_name):
                ...
                try:
                    ...
                    print 'WARNING: error renaming some wave files' 
                try:
                    utils.ensure_file_exists(b_file + '.ev_is.seg')
                except IOError:
                    clu._generate_a_seg_file(b_file + '.ev_is.seg', b_file) #generate a seg file for the current cluster
...
#        """
    ...
    def write_output(self, mode):
        ...
        if mode == 'srt':
            ...
            try:
                fm.seg2srt(self.get_file_basename() + '.ev_is.seg')
            except:
...
def manage_ident(filebasename, gmm, clusters):
    ...
    print 'filename', filebasename
    print 'gmm', gmm
    print 'clusters', clusters
    seg_f = open("%s.ident.%s.ev_is.seg" % (filebasename, gmm), "r")
    for line in seg_f:
        if line.startswith(";;"):
            print 'line', line
            splitted_line = line.split(' ')[2].split('_')
            ...
            clusters[cluster].add_speaker(speaker+'_'+gmm.split('.')[-2], value)
    seg_f.close()
    if not CONFIGURATION.KEEP_INTERMEDIATE_FILES:
        os.remove("%s.ident.%s.ev_is.seg" % (filebasename, gmm))
